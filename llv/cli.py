...
def playback_pose(host, port, filepath, fps, loop=True):
    fps = clamp(fps, 1, 76)  # https://stackoverflow.com/a/1133888
    ...
    frame_index = -1
    frame_count = -1
    for frame_package in read_frames_pose(filepath, loop=loop):
        frame_data, frame_index, frame_count, version = frame_package
        if 0 == frame_index:
            logger.info(
                f"Start sending {frame_count} frames of version {version} @{fps}fps ..."
...
def record(host, port, frames, output, with_raw_frame=False):
    ...
    with gzip.open(output, "wb") as file:
        ...
        while current_data_frame < frames:
            ...
            print(
                f'Processing frame {current_data_frame+1} ({frame.frame_time["frame_number"]}) ...'
            )
...
def unpack(raw_file, output, retain_raw_frame=True, rename=""):
    with open(output, "w", encoding="utf-8", newline="\r\n") as file:
        frame_index = -1
        frame_count = -1
        for frame_data, frame_index, frame_count, version in read_frames(
            raw_file, loop=False
        ):
            if 0 == frame_index:
...
def pack(clear_filepath, output, rename=""):
    print(
        f"Generating packed recording at {clear_filepath} from clearfile {clear_filepath} ..."
    )
    with gzip.open(output, "wb") as outfile:
        ...
        for frame_json, frame_index, frame_count, version in read_frames(
            clear_filepath, is_binary=False, loop=False
        ):
            if 0 == frame_index:
...
def _write_frames_for_shape(
    ...
    for shape_frame_index in range(0, frames_per_shape):
        ...
        frame.blendshapes[shape_name] = remap(
            shape_frame_index, 0, frames_per_shape - 1, min_value, max_value
        )
...
def sequence(
    output, time_per_shape=1.1, fps=60, single_shape="", min_value=-1.0, max_value=1.0
):
    ...
    total_number_of_shapes = (
        1 if 0 < len(single_shape) else len(FaceFrame.FACE_BLENDSHAPE_NAMES)
    )
    ...
    with gzip.open(output, "wb") as file:
        ...
        file.write(
            struct.pack(">L", total_number_of_frames)
        )  # how many frames are in the recording?
        ...
        if 0 < len(single_shape):
            ...
            if not single_shape in FaceFrame.FACE_BLENDSHAPE_NAMES:
                raise Exception(
                    f"Could not find {single_shape} in shape blendshape defintion!"
                )
            _write_frames_for_shape(
                file,
                single_shape,
                frames_per_shape,
                total_number_of_shapes,
...
def apply_modifiers(recording_filepath, modifiers_filepath, default_value=1.0):
    ...
    for frame_data, frame_index, frame_count, version in read_frames(
        recording_filepath, is_binary=is_binary_recording, loop=False
    ):
        ...
        for shape in fbx_metadata["shapes"]:
            ...
            if l in arr:
                ...
                shape_values[arkit_name] = [
                    library["modifiers"][arkit_name] * value
                    for value in shape["curves"][0]["values"]
                ]
                max_len_values = max(max_len_values, len(shape_values[arkit_name]))
                max_duration = max(
                    max_duration,
                    shape["curves"][0]["end_time"] - shape["curves"][0]["start_time"],
                )

    tween_frames = math.floor(60.0 / math.floor(max_len_values / max_duration))
    if 1 > tween_frames:
        raise Exception(
            f"Weirdness! 1 > (60 / ({max_len_values} (max_len_values) / {max_duration} (max_duration)))"
        )
            ...
            for shape_name in FaceFrame.FACE_BLENDSHAPE_NAMES:
                ...
                if shape_name in shape_values:
                    try:
                        ...
                        print(
                            f"Got {e} when accessing {frame_index} / len {len(shape_values[shape_name])}"
                        )
                        raise e
...
def create_arg_parser():
    ...
    record_args.add_argument(
        "--port", metavar="p", type=int, help="Port to host server on.", default=11111
    )
    record_args.add_argument(
        "--frames", metavar="f", type=int, help="Frame count to record.", default=300
    )
    record_args.add_argument(
        "--with-raw",
        action="store_true",
    ...
    play_args.add_argument(
        "recording_path", metavar="in_path", type=str, help="Path to recording file."
    )
    play_args.add_argument(
        "--fps",
        metavar="f",
    ...
    play_args.add_argument(
        "--port", metavar="p", type=int, help="Port to target.", default=11111
    )

    # Setup play command and options.
    playpose_args = subparsers.add_parser("playpose")
    playpose_args.add_argument(
        "recording_path", metavar="in_path", type=str, help="Path to recording file."
    )
    playpose_args.add_argument(
        "--fps",
    ...
    remap_args.add_argument(
        "fbx_meta_filepath",
        metavar="fbx_meta_filepath",
        type=str,
        help="Path to fbx blendshape metadata file.",
    )
    remap_args.add_argument(
        "library_filepath",
        metavar="library_filepath",
        type=str,
...
def main():
    ...
    if "play" == args.command:
        frames_read, frames_total = playback(
            args.host, args.port, args.recording_path, args.fps
        )
        print(f"Stopped at frame {frames_read}/{frames_total}")
    elif "playpose" == args.command:
        frames_read, frames_total = playback_pose(
            args.host, args.port, args.recording_path, args.fps
        )
        print(f"Stopped at frame {frames_read}/{frames_total}")
    elif "record" == args.command:
        frames_read, frames_requested, filepath = record(
            args.host, args.port, args.frames, args.output, args.with_raw
        )
        print(
            f"Stopped at frame {frames_read}/{frames_requested}, written file to {filepath}"
        )
